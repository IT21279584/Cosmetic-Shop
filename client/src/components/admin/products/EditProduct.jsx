...
const EditProduct = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const newImagesCount = imageFiles.filter((file) => file).length;
    ...
    try {
      ...
      await adminService.updateProduct(id, submitFormData);
  ...
  const totalImages =
    existingImages.length + imagePreviews.filter((p) => p).length;
        ...
        <div className="overflow-hidden border shadow-xl border-primary-100 bg-white/80 backdrop-blur-sm rounded-2xl">
          ...
          <div className="p-4 border-b border-primary-100 sm:p-6 lg:p-8 bg-gradient-to-r ">
            ...
            <p className="mb-4 text-xs font-medium sm:text-sm">
              <span
                className={
                  totalImages < 4 ? "text-orange-600" : "text-green-600"
                }
              >
                Current: {totalImages}/4 images
              </span>
              {totalImages < 4 && (
                <span className="ml-2 text-orange-600">
                  ({4 - totalImages} more recommended)
                </span>
              )}
              {totalImages === 4 && (
                ...
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 sm:gap-4">
                  {existingImages.map((imageUrl, index) => (
                    <div key={`existing-${index}`} className="relative group">
                      <div className="p-3 transition-all duration-300 border-2 border-primary-200 rounded-xl sm:p-4 hover:border-primary-400 hover:bg-primary-50/50">
            ...
            {availableSlots > 0 && (
              <div>
                <h4 className="mb-3 text-sm font-semibold text-gray-700">
                  Add New Images ({availableSlots} slots available)
                </h4>
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 sm:gap-4">
